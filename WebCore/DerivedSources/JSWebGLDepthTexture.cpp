...
static const std::array<HashTableValue, 1> JSWebGLDepthTexturePrototypeTableValues {
    HashTableValue { "UNSIGNED_INT_24_8_WEBGL"_s, PropertyAttribute::ReadOnly | PropertyAttribute::DontDelete | PropertyAttribute::ConstantInteger, NoIntrinsic, { HashTableValue::ConstantType, 0x84FA } },
};
...
{
    return WebCore::subspaceForImpl<JSWebGLDepthTexture, UseCustomHeapCellType::No>(vm, "JSWebGLDepthTexture"_s,
...
bool JSWebGLDepthTextureOwner::isReachableFromOpaqueRoots(JSC::Handle<JSC::Unknown> handle, void*, AbstractSlotVisitor& visitor, ASCIILiteral* reason)
{
    auto* jsWebGLDepthTexture = jsCast<JSWebGLDepthTexture*>(handle.slot()->asCell());
    auto& owner = jsWebGLDepthTexture->wrapped();
    if (reason) [[unlikely]]
        *reason = "Reachable from jsWebGLDepthTexture"_s;
...
void JSWebGLDepthTextureOwner::finalize(JSC::Handle<JSC::Unknown> handle, void* context)
{
    ...
    uncacheWrapper(world, jsWebGLDepthTexture->protectedWrapped().ptr(), jsWebGLDepthTexture);
}
...
#endif
template<std::same_as<WebGLDepthTexture> T>
static inline void verifyVTable(WebGLDepthTexture* ptr) 
{
...
{
#if ENABLE(BINDING_INTEGRITY)
    verifyVTable<WebGLDepthTexture>(impl.ptr());
#endif
