...
template<> EncodedJSValue JSC_HOST_CALL_ATTRIBUTES JSBackgroundFetchUpdateUIEventDOMConstructor::construct(JSGlobalObject* lexicalGlobalObject, CallFrame* callFrame)
{
    ...
    auto initConversionResult = convert<IDLDictionary<BackgroundFetchEventInit>>(*lexicalGlobalObject, argument1.value());
    if (initConversionResult.hasException(throwScope)) [[unlikely]]
       return encodedJSValue();
    auto object = BackgroundFetchUpdateUIEvent::create(typeConversionResult.releaseReturnValue(), initConversionResult.releaseReturnValue());
...
static const std::array<HashTableValue, 2> JSBackgroundFetchUpdateUIEventPrototypeTableValues {
    HashTableValue { "constructor"_s, static_cast<unsigned>(PropertyAttribute::DontEnum), NoIntrinsic, { HashTableValue::GetterSetterType, jsBackgroundFetchUpdateUIEventConstructor, 0 } },
    HashTableValue { "updateUI"_s, static_cast<unsigned>(JSC::PropertyAttribute::Function), NoIntrinsic, { HashTableValue::NativeFunctionType, jsBackgroundFetchUpdateUIEventPrototypeFunction_updateUI, 0 } },
};
...
JSC_DEFINE_CUSTOM_GETTER(jsBackgroundFetchUpdateUIEventConstructor, (JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName))
{
    ...
    return JSValue::encode(JSBackgroundFetchUpdateUIEvent::getConstructor(vm, prototype->globalObject()));
}
...
static inline JSC::EncodedJSValue jsBackgroundFetchUpdateUIEventPrototypeFunction_updateUIBody(JSC::JSGlobalObject* lexicalGlobalObject, JSC::CallFrame* callFrame, typename IDLOperationReturningPromise<JSBackgroundFetchUpdateUIEvent>::ClassParameter castedThis, Ref<DeferredPromise>&& promise)
{
    ...
    auto optionsConversionResult = convert<IDLDictionary<BackgroundFetchUIOptions>>(*lexicalGlobalObject, argument0.value());
    if (optionsConversionResult.hasException(throwScope)) [[unlikely]]
       return encodedJSValue();
    RELEASE_AND_RETURN(throwScope, JSValue::encode(toJS<IDLPromise<IDLUndefined>>(*lexicalGlobalObject, *castedThis->globalObject(), throwScope, [&]() -> decltype(auto) { return impl.updateUI(optionsConversionResult.releaseReturnValue(), WTFMove(promise)); })));
}
...
{
    return WebCore::subspaceForImpl<JSBackgroundFetchUpdateUIEvent, UseCustomHeapCellType::No>(vm, "JSBackgroundFetchUpdateUIEvent"_s,
...
#endif
template<std::same_as<BackgroundFetchUpdateUIEvent> T>
static inline void verifyVTable(BackgroundFetchUpdateUIEvent* ptr) 
{
...
{
#if ENABLE(BINDING_INTEGRITY)
    verifyVTable<BackgroundFetchUpdateUIEvent>(impl.ptr());
#endif
