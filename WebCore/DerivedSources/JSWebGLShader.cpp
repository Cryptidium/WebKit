...
static const std::array<HashTableValue, 1> JSWebGLShaderPrototypeTableValues {
    HashTableValue { "constructor"_s, static_cast<unsigned>(PropertyAttribute::DontEnum), NoIntrinsic, { HashTableValue::GetterSetterType, jsWebGLShaderConstructor, 0 } },
};
...
JSC_DEFINE_CUSTOM_GETTER(jsWebGLShaderConstructor, (JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName))
{
    ...
    return JSValue::encode(JSWebGLShader::getConstructor(vm, prototype->globalObject()));
}

JSC::GCClient::IsoSubspace* JSWebGLShader::subspaceForImpl(JSC::VM& vm)
{
    return WebCore::subspaceForImpl<JSWebGLShader, UseCustomHeapCellType::No>(vm, "JSWebGLShader"_s,
...
bool JSWebGLShaderOwner::isReachableFromOpaqueRoots(JSC::Handle<JSC::Unknown> handle, void*, AbstractSlotVisitor& visitor, ASCIILiteral* reason)
{
    auto* jsWebGLShader = jsCast<JSWebGLShader*>(handle.slot()->asCell());
    WebGLShader* owner = &jsWebGLShader->wrapped();
    if (reason) [[unlikely]]
        *reason = "Reachable from WebGLShader"_s;
...
void JSWebGLShaderOwner::finalize(JSC::Handle<JSC::Unknown> handle, void* context)
{
    ...
    uncacheWrapper(world, jsWebGLShader->protectedWrapped().ptr(), jsWebGLShader);
}
...
#endif
template<std::same_as<WebGLShader> T>
static inline void verifyVTable(WebGLShader* ptr) 
{
...
{
#if ENABLE(BINDING_INTEGRITY)
    verifyVTable<WebGLShader>(impl.ptr());
#endif
