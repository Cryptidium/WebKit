...
template<> ConversionResult<IDLDictionary<StereoPannerOptions>> convertDictionary<StereoPannerOptions>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    JSValue panValue;
    if (isNullOrUndefined)
        panValue = jsUndefined();
    else {
        panValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "pan"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    auto panConversionResult = convertOptionalWithDefault<IDLFloat>(lexicalGlobalObject, panValue, [&]() -> ConversionResult<IDLFloat> { return Converter<IDLFloat>::ReturnType { 0 }; });
    if (panConversionResult.hasException(throwScope)) [[unlikely]]
        return ConversionResultException { };
    result.pan = panConversionResult.releaseReturnValue();
    return result;
}
