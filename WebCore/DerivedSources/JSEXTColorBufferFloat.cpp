...
{
    return WebCore::subspaceForImpl<JSEXTColorBufferFloat, UseCustomHeapCellType::No>(vm, "JSEXTColorBufferFloat"_s,
...
bool JSEXTColorBufferFloatOwner::isReachableFromOpaqueRoots(JSC::Handle<JSC::Unknown> handle, void*, AbstractSlotVisitor& visitor, ASCIILiteral* reason)
{
    auto* jsEXTColorBufferFloat = jsCast<JSEXTColorBufferFloat*>(handle.slot()->asCell());
    auto& owner = jsEXTColorBufferFloat->wrapped();
    if (reason) [[unlikely]]
        *reason = "Reachable from jsEXTColorBufferFloat"_s;
...
void JSEXTColorBufferFloatOwner::finalize(JSC::Handle<JSC::Unknown> handle, void* context)
{
    ...
    uncacheWrapper(world, jsEXTColorBufferFloat->protectedWrapped().ptr(), jsEXTColorBufferFloat);
}
...
#endif
template<std::same_as<EXTColorBufferFloat> T>
static inline void verifyVTable(EXTColorBufferFloat* ptr) 
{
...
{
#if ENABLE(BINDING_INTEGRITY)
    verifyVTable<EXTColorBufferFloat>(impl.ptr());
#endif
