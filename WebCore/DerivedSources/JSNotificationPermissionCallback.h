...
public:
    ...
    ~JSNotificationPermissionCallback() final;
    JSCallbackData* callbackData() { return m_data; }

    // Functions
    CallbackResult<typename IDLUndefined::CallbackReturnType> invoke(typename IDLEnumeration<NotificationPermission>::ParameterType permission) override;
    CallbackResult<typename IDLUndefined::CallbackReturnType> invokeRethrowingException(typename IDLEnumeration<NotificationPermission>::ParameterType permission) override;
...
template<> struct JSDOMCallbackConverterTraits<JSNotificationPermissionCallback> {
    using Base = NotificationPermissionCallback;
};
} // namespace WebCore
