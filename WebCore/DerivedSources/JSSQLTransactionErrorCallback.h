...
public:
    ...
    ~JSSQLTransactionErrorCallback() final;
    JSCallbackData* callbackData() { return m_data; }

    // Functions
    CallbackResult<typename IDLUndefined::CallbackReturnType> invoke(typename IDLInterface<SQLError>::ParameterType error) override;
    CallbackResult<typename IDLUndefined::CallbackReturnType> invokeRethrowingException(typename IDLInterface<SQLError>::ParameterType error) override;
...
template<> struct JSDOMCallbackConverterTraits<JSSQLTransactionErrorCallback> {
    using Base = SQLTransactionErrorCallback;
};
} // namespace WebCore
