...
template<> ConversionResult<IDLDictionary<CookieStoreGetOptions>> convertDictionary<CookieStoreGetOptions>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
...
JSC::JSObject* convertDictionaryToJS(JSC::JSGlobalObject& lexicalGlobalObject, JSDOMGlobalObject& globalObject, const CookieStoreGetOptions& dictionary)
{
    SUPPRESS_UNCOUNTED_LOCAL auto& vm = JSC::getVM(&lexicalGlobalObject);
    auto throwScope = DECLARE_THROW_SCOPE(vm);
    ...
    if (!IDLUSVString::isNullValue(dictionary.url)) {
        auto urlValue = toJS<IDLUSVString>(lexicalGlobalObject, throwScope, IDLUSVString::extractValueFromNullable(dictionary.url));
        RETURN_IF_EXCEPTION(throwScope, { });
