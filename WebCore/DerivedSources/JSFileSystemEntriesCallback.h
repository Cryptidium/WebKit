...
public:
    ...
    ~JSFileSystemEntriesCallback() final;
    JSCallbackData* callbackData() { return m_data; }

    // Functions
    CallbackResult<typename IDLUndefined::CallbackReturnType> invoke(typename IDLSequence<IDLInterface<FileSystemEntry>>::ParameterType entries) override;
    CallbackResult<typename IDLUndefined::CallbackReturnType> invokeRethrowingException(typename IDLSequence<IDLInterface<FileSystemEntry>>::ParameterType entries) override;
...
template<> struct JSDOMCallbackConverterTraits<JSFileSystemEntriesCallback> {
    using Base = FileSystemEntriesCallback;
};
} // namespace WebCore
