...
{
    return WebCore::subspaceForImpl<JSEXTFloatBlend, UseCustomHeapCellType::No>(vm, "JSEXTFloatBlend"_s,
...
bool JSEXTFloatBlendOwner::isReachableFromOpaqueRoots(JSC::Handle<JSC::Unknown> handle, void*, AbstractSlotVisitor& visitor, ASCIILiteral* reason)
{
    auto* jsEXTFloatBlend = jsCast<JSEXTFloatBlend*>(handle.slot()->asCell());
    auto& owner = jsEXTFloatBlend->wrapped();
    if (reason) [[unlikely]]
        *reason = "Reachable from jsEXTFloatBlend"_s;
...
void JSEXTFloatBlendOwner::finalize(JSC::Handle<JSC::Unknown> handle, void* context)
{
    ...
    uncacheWrapper(world, jsEXTFloatBlend->protectedWrapped().ptr(), jsEXTFloatBlend);
}
...
#endif
template<std::same_as<EXTFloatBlend> T>
static inline void verifyVTable(EXTFloatBlend* ptr) 
{
...
{
#if ENABLE(BINDING_INTEGRITY)
    verifyVTable<EXTFloatBlend>(impl.ptr());
#endif
