...
template<> ConversionResult<IDLDictionary<ExtendableCookieChangeEventInit>> convertDictionary<ExtendableCookieChangeEventInit>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    ExtendableCookieChangeEventInit result;
    ...
    JSValue changedValue;
    if (isNullOrUndefined)
        changedValue = jsUndefined();
    else {
        changedValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "changed"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (!changedValue.isUndefined()) {
        auto changedConversionResult = convert<IDLSequence<IDLDictionary<CookieListItem>>>(lexicalGlobalObject, changedValue);
        if (changedConversionResult.hasException(throwScope)) [[unlikely]]
            return ConversionResultException { };
        result.changed = changedConversionResult.releaseReturnValue();
    }
    JSValue deletedValue;
    if (isNullOrUndefined)
        deletedValue = jsUndefined();
    else {
        deletedValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "deleted"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (!deletedValue.isUndefined()) {
        auto deletedConversionResult = convert<IDLSequence<IDLDictionary<CookieListItem>>>(lexicalGlobalObject, deletedValue);
        if (deletedConversionResult.hasException(throwScope)) [[unlikely]]
            return ConversionResultException { };
        result.deleted = deletedConversionResult.releaseReturnValue();
    }
    return result;
}
