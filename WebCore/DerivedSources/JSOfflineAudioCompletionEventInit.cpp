...
template<> ConversionResult<IDLDictionary<OfflineAudioCompletionEventInit>> convertDictionary<OfflineAudioCompletionEventInit>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    OfflineAudioCompletionEventInit result;
    ...
    JSValue renderedBufferValue;
    if (isNullOrUndefined)
        renderedBufferValue = jsUndefined();
    else {
        renderedBufferValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "renderedBuffer"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (renderedBufferValue.isUndefined()) {
        throwRequiredMemberTypeError(lexicalGlobalObject, throwScope, "renderedBuffer"_s, "OfflineAudioCompletionEventInit"_s, "AudioBuffer"_s);
        return ConversionResultException { };
    }
    auto renderedBufferConversionResult = convert<IDLInterface<AudioBuffer>>(lexicalGlobalObject, renderedBufferValue);
    if (renderedBufferConversionResult.hasException(throwScope)) [[unlikely]]
        return ConversionResultException { };
    result.renderedBuffer = renderedBufferConversionResult.releaseReturnValue();
    return result;
}
