...
template<> ConversionResult<IDLDictionary<PushSubscriptionChangeEventInit>> convertDictionary<PushSubscriptionChangeEventInit>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    PushSubscriptionChangeEventInit result;
    ...
    JSValue newSubscriptionValue;
    if (isNullOrUndefined)
        newSubscriptionValue = jsUndefined();
    else {
        newSubscriptionValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "newSubscription"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (!newSubscriptionValue.isUndefined()) {
        auto newSubscriptionConversionResult = convert<IDLNullable<IDLInterface<PushSubscription>>>(lexicalGlobalObject, newSubscriptionValue);
        if (newSubscriptionConversionResult.hasException(throwScope)) [[unlikely]]
            return ConversionResultException { };
        result.newSubscription = newSubscriptionConversionResult.releaseReturnValue();
    }
    JSValue oldSubscriptionValue;
    if (isNullOrUndefined)
        oldSubscriptionValue = jsUndefined();
    else {
        oldSubscriptionValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "oldSubscription"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (!oldSubscriptionValue.isUndefined()) {
        auto oldSubscriptionConversionResult = convert<IDLNullable<IDLInterface<PushSubscription>>>(lexicalGlobalObject, oldSubscriptionValue);
        if (oldSubscriptionConversionResult.hasException(throwScope)) [[unlikely]]
            return ConversionResultException { };
        result.oldSubscription = oldSubscriptionConversionResult.releaseReturnValue();
    }
    return result;
}
