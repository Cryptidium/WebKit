...
#include "JSObservableInspectorAbortCallback.h"
...
template<> ConversionResult<IDLDictionary<ObservableInspector>> convertDictionary<ObservableInspector>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    ObservableInspector result;
    JSValue abortValue;
    if (isNullOrUndefined)
        abortValue = jsUndefined();
    else {
        abortValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "abort"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (!abortValue.isUndefined()) {
        auto abortConversionResult = convert<IDLCallbackFunction<JSObservableInspectorAbortCallback>>(lexicalGlobalObject, abortValue, *jsCast<JSDOMGlobalObject*>(&lexicalGlobalObject));
        if (abortConversionResult.hasException(throwScope)) [[unlikely]]
            return ConversionResultException { };
        result.abort = abortConversionResult.releaseReturnValue();
    }
    ...
    JSValue subscribeValue;
    if (isNullOrUndefined)
        subscribeValue = jsUndefined();
    else {
        subscribeValue = object->get(&lexicalGlobalObject, Identifier::fromString(vm, "subscribe"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    if (!subscribeValue.isUndefined()) {
        auto subscribeConversionResult = convert<IDLCallbackFunction<JSVoidCallback>>(lexicalGlobalObject, subscribeValue, *jsCast<JSDOMGlobalObject*>(&lexicalGlobalObject));
        if (subscribeConversionResult.hasException(throwScope)) [[unlikely]]
            return ConversionResultException { };
        result.subscribe = subscribeConversionResult.releaseReturnValue();
    }
    return result;
}
