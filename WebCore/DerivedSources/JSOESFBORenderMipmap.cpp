...
{
    return WebCore::subspaceForImpl<JSOESFBORenderMipmap, UseCustomHeapCellType::No>(vm, "JSOESFBORenderMipmap"_s,
...
bool JSOESFBORenderMipmapOwner::isReachableFromOpaqueRoots(JSC::Handle<JSC::Unknown> handle, void*, AbstractSlotVisitor& visitor, ASCIILiteral* reason)
{
    auto* jsOESFBORenderMipmap = jsCast<JSOESFBORenderMipmap*>(handle.slot()->asCell());
    auto& owner = jsOESFBORenderMipmap->wrapped();
    if (reason) [[unlikely]]
        *reason = "Reachable from jsOESFBORenderMipmap"_s;
...
void JSOESFBORenderMipmapOwner::finalize(JSC::Handle<JSC::Unknown> handle, void* context)
{
    ...
    uncacheWrapper(world, jsOESFBORenderMipmap->protectedWrapped().ptr(), jsOESFBORenderMipmap);
}
...
#endif
template<std::same_as<OESFBORenderMipmap> T>
static inline void verifyVTable(OESFBORenderMipmap* ptr) 
{
...
{
#if ENABLE(BINDING_INTEGRITY)
    verifyVTable<OESFBORenderMipmap>(impl.ptr());
#endif
